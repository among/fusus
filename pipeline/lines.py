...
def getInkDistribution(C, info, stages, blocks, batch, boxed):
    ...
    for ((stripe, column), data) in blocks.items():
        ...
        lines = getInkY(C, info, rotated, left, top, right, bottom, True, imgOut=imgOut)
...
def getInkY(C, info, imgIn, left, top, right, bottom, final, imgOut=None):
    ...
    Note that the parameter `pipeline.parameters.lineHeight` will get updated
    ...
    if show:
        ...
        info(f"peaks={[f'{histY[peak]} @ {peak}' for peak in peaks]}", tm=False)
