...
"url":4,
"doc":"Metadata of works in the pipeline. Here are the main works and the commentaries. They are given a keyword, and under that keyword there is metadata, the source directory, the destination directory for Text-Fabric files, whether the work has a table of contents, and whether the work is the outcome of an OCR process. Functions that deal with works can be given the keyword as an argument, and then they know how to deal with its data."
},
{
...
{
"ref":"fusus.tsv.loadTsv",
"url":8,
"doc":"Load a tsv file into memory. The tsv file either comes from a known work or is specified by a path. If it comes from a known work, you only hace to pass the key of that work, e.g.  fususa , or  fususl . The function needs to know whether the tsv comes out of an OCR process or from the text extraction of a PDF. In case of a known work, this is known and does not have to be specified. Otherwise you have to pass it. Parameters      source: string, optional  None The key of a known work, see  fusus.works.WORKS . path: string The path to the tsv file. Not needed in case of a known work. ocred: string Whether the tsv file comes from the OCR pipeline. Not needed in case of a known work. pages: string|int, optional  None A specification of zero or more page numbers (see  fusus.lib.parseNums ). Only rows belonging to selected pages will be extracted. If None, all pages will be taken. Returns    - tuple Two members:  head: a tuple of field names  rows: a tuple of data rows, where each row is a tuple of its fields",
"func":1
},
{
"ref":"fusus.pdf",
