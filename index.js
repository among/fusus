...
"url":12,
"doc":"This is a technical overview of the capabilities of the fusus package. We discuss the main problems we encountered and how we solved them, pointing to relevant technologies. The  fusus package deals with two distinct problems:  get text from page images by means of Optical Character Recognition  extract text from a textual, but convoluted PDF  OCR Pipeline We have to recognize page images from Arabic books over several centuries. Initial experiments showed that Tesseract and its Arabic model did not do a good job on these pages. It turned out that Kraken did a much better job, using a model trained by others on Arabic texts. See below.  Preprocessing Yet we needed considerable preprocessing to aid and feed the OCR engine. The tool of the trade for that is [OpenCV](https: opencv-python-tutroals.readthedocs.io/en/latest/py_tutorials/py_tutorials.html). The added beauty of this is that OpenCV represents images as [NumPy](https: numpy.org/doc/stable/reference/index.html) arrays and allows to manipulate images under the hood by NumPy operations. This is what we do frequently. When things get less straightforward, we can also make use of the signal processing library within the [SciPy](https: docs.scipy.org/doc/scipy/reference/) library.  Normalizing Page images need to be normalized, i.e. turned into gray-scale and then black-and white. If there are black borders, they need to be removed. A first attempt was to make the edges of the page white, using a fixed proportion of the page dimensions to determine what counts as a border. However, if we adjust the border widths to the extent that it removes all known black borders, it will also clip some real text on other pages. Now we do it dynamically: we detect where the borders are by surprisingly straightforward Numpy operations. See  fusus.lib.cropBorders . We also blur, invert and threshold the image, in order to make useful histograms of the ink distribution on the page.  Mark removal In order to remove unwanted marks that occur repeatedly we use [template matching](https: opencv-python-tutroals.readthedocs.io/en/latest/py_tutorials/py_imgproc/py_template_matching/py_template_matching.html template-matching) by OpenCV to spot them.  Layout detection  Line detection  OCR application  Proofing  PDF Text Extraction  Double characters  Private use characters  Reading order  Word separation  Text reconstruction  Delivery  TSV files  Text-Fabric datasets"
},
{
